...
'''
def listconv_str_int(string):
    int_tupla = []
    num=''
    for i in range(len(string)):
        if string[i] in '0123456789':
            num+=string[i]
        if (string[i]==',' or i == len(string)-1) and num!='':
            int_tupla.append(int(num))
            num=''
    return int_tupla
    pass
...
def ex1(int_seq, subtotal):
    ...
    int_seq = listconv_str_int(int_seq)
    output = 0
    somma = 0
    dict_somma = {0: 1}
    for i in range(len(int_seq)):
        somma += int_seq[i]
        diff = somma - subtotal
        if diff in dict_somma:
            output += dict_somma[diff]
        if somma in dict_somma:
            dict_somma[somma] += 1
        else:
            dict_somma[somma] = 1
    return output
