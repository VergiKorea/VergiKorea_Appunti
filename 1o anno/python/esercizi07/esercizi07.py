...
def search_word(filename: str, word: str) -> int:
    f = load_file(filename).lower()
    punteg = f.maketrans('!",.\n', '     ')
    return f.translate(punteg).split().count(word)
...
def load_table(filename: str) -> List[Dict[str, str]]:
    output = []
    string = load_file(filename).split('\n')
    keys = string[0].split()
    for i in range(1, len(string)):
        dict_row = {}
        row = string[i].split()
        for j in range(len(row)):
            dict_row[keys[j]] = row[j]
...
def load_data(filename: str) -> List[Dict[str, Union[int, str]]]:
    ...
    for i in range(len(table)):
        for key in table[i]:
            if table[i][key].isdigit() == True:
                table[i][key] = int(table[i][key])
    return table
    ...
    for i in range(1, len(string)):
        ...
        for j in range(len(row)):
            if row[j].isdigit() == True:
                dict_row[keys[j]] = int(row[j])
            elif empties == True and row[j] == '':
                dict_row[keys[j]] = None
            elif row[j]!='':
                dict_row[keys[j]] = row[j]
        output.append(dict_row)
    return output
