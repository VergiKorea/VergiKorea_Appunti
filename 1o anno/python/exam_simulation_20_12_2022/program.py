...
def func1(string_list):
    def isCapital(string):
        if string[0].isupper() == True:
            ...
            return False
    return sorted(list(filter(isCapital, string_list)), key=len)
...
'''
def func2(pathname):
    with open(pathname, encoding='utf-8') as p:
        txt = p.read().split('\n')
    output_dict = {}
    for row in txt:
        if row.split()[0] in output_dict:
            output_dict[row.split()[0]].add(row.split()[1])
        else:
            output_dict[row.split()[0]] = set([row.split()[1]])
...
def func3(listA, listB, listC, pathname):
    listD = [(listA[i]+listB[i])*listC[i] for i in range(len(listA))]
    strD = ''
    for num in listD:
        strD+=f'{num}\n'
...
def func4(triangles):
    ...
    for triangle in triangles:
        ...
        listTriangle.sort()
        if round(listTriangle[0]**2 + listTriangle[1]**2, 3) != round(listTriangle[2]**2, 3):
            rem+=1
        else:
            isRight.append(triangle)
    triangles = isRight
    return rem
...
def ex1(directory, namefile):
    ...
    con=0
    for dir_file in subdirs:
        if namefile in dir_file:
            with open(f'{directory}/{dir_file}', encoding='utf-8') as p:
                txt = p.read().split()
            ...
            if len(out_sum) == 0:
                ...
                for i in range(len(out_sum)):
                    out_sum[i]+=txt[i]
        elif '.txt' not in dir_file:
            con+=1
            out_sum2 = ex1(f'{directory}/{dir_file}', namefile)
            if len(out_sum) == 0:
                out_sum = [0]*len(out_sum2)
            for i in range(len(out_sum)):
                out_sum[i]+=out_sum2[i]
    if len(out_sum) == 0:
        return [0,0,0]
    else:
        return out_sum
...
def ex2(strings, n):
    ...
    if n == 1:
        ...
        output = set()
        for el1 in strings:
            elms_riman = strings - {el1}
            for el2 in ex2(elms_riman, n-1):
                output.add(f'{el1}{el2}')
        return sorted(output, key = lambda el: (-len(el), el))
