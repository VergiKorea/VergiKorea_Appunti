...
def generate_digits(bases : List[int] ) -> List[List[int]]:
    ...
    # SCRIVI QUI IL TUO CODICE
    num_combinazioni = 1
    for x in bases: num_combinazioni*=x
    combinazioni = []
    combinazione = [0]*len(bases)
    
    while len(combinazioni) < num_combinazioni:
        combinazioni.append(combinazione[:])
        combinazione[0]+=1
        for i in range(len(bases)):
            if combinazione[i] >= bases[i]:
                combinazione[i] = 0
                if (i)<len(bases)-1:
                    combinazione[i+1]+=1
...
def find_doubles(bases : List[int]) -> Set[int]:
    ...
    comb_int = []
    comb_int_doppi = []
    for combinazione in generate_digits(bases):
        if decode_digits(combinazione, bases) in comb_int: comb_int_doppi.append(decode_digits(combinazione, bases))
        comb_int.append(decode_digits(combinazione, bases))
    return set(comb_int_doppi)
