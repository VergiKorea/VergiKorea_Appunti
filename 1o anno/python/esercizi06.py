...
def reverse_recursive(s: str) -> str:
    if len(s) == 1:
        ...
        return f'{reverse_recursive(s[1:])}{s[0]}'
...
def knapsack(l: List[Tuple[int, int]], W: int) -> int:
    all_prizes=[ogg[1] for ogg in l]
    vMax_index = all_prizes.index(max(all_prizes))
    if l[vMax_index][0]>W:
        return 0
    elif l[vMax_index][0]==W:
        return l[vMax_index][1]
    else:
        return l[vMax_index][1]+knapsack(l[:vMax_index]+l[vMax_index+1:], W-l[vMax_index][0])
