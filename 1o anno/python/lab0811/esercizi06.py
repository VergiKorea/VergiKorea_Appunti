...
def tribonacci(n: int):
    lista_tribonacci = [0,0,1,1]
    if n>2: return tribonacci(n-1) + tribonacci(n-2) + tribonacci(n-3)
    else: return lista_tribonacci[n]
...
def reverse_recursive(s: str) -> str:
    if len(s)<=1:
        ...
        return s[len(s)-1] + reverse_recursive(s[:len(s)-1])
...
def knapsack(l: List[Tuple[int, int]], W: int) -> int:
    ...
    if w_max<W:
        v_sum+=v_max
        l.remove((w_max, v_max))
        return v_sum+knapsack(l, W-w_max)
