...
def decrypt(text, pharaohs_cypher):
    list_decrypt = []
    for key in pharaohs_cypher:
        for i in range(len(text)-len(key)):
            substring = text[i:i+len(key)+1]
            mem_key = [*key]
            if len([char for char in substring if char not in mem_key or mem_key.remove(char)]) == 1:
                sub_list = decrypt(f'{text[:i]}{pharaohs_cypher[key]}{text[i+len(key)+1:]}', pharaohs_cypher)
                if len(sub_list) == 0:
                    list_decrypt.append(f'{text[:i]}{pharaohs_cypher[key]}{text[i+len(key)+1:]}')
                else:
                    list_decrypt.extend(sub_list)
    list_decrypt = set(list_decrypt)
    return list_decrypt
    
def pharaohs_revenge(encrypted_text : str, pharaohs_cypher : dict[str,str]) -> set[str]:
    output = decrypt(encrypted_text, pharaohs_cypher)
    minimum = min(len(string) for string in output)
    return set(string for string in output if len(string) == minimum)
