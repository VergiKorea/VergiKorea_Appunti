...
def es42(fImageIn, fcolori, fImageOut):
    ...
    with open(fcolori, encoding='utf-8') as f:
        flist = [row.split()for row in f.read().split('\n')]
    RGB_dict = {}
    for row in flist:
        RGB_dict[tuple([int(num) for num in row[:3]])] = tuple([int(num) for num in row[3:6]])
    cnt = 0
    imgin = immagini.load(fImageIn)
    for r in range(len(imgin)):
        for c in range(len(imgin[0])):
            if imgin[r][c] in [key for key in RGB_dict]:
                cnt+=1
                imgin[r][c] = RGB_dict[imgin[r][c]]
    immagini.save(imgin, fImageOut)
    return cnt
    

print(es42('scacchiera.png', 'fcolori1.txt', 'out1.png'))
