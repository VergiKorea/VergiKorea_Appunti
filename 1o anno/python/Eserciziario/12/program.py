...
def es1(k):
    ...
    def crea_albero(k, n):
        if k == 1:
            sx = n
            dx = n+1
            nodo = albero.Nodo(sx+dx)
            nodo.f = [albero.Nodo(sx), albero.Nodo(dx)]
            return nodo
        else:
            sx = crea_albero(k-1, n)
            dx = crea_albero(k-1, n+(2**(k-1)))
            nodo = albero.Nodo(sx.id+dx.id)
            nodo.f = [sx,dx]
            return nodo
            
            
        
    return crea_albero(k, 1)
    

print(albero.toLista(es1(3)))
