...
def check_pwd(pwd: str) -> bool:
    if len(pwd)>=6 and len(pwd)<=16 and len([char for char in pwd if char.islower()==True])>0 and len([char for char in pwd if char.isupper()==True])>0 and len([char for char in pwd if char in '1234567890'])>0 and len([char for char in pwd if char in '$#@'])>0 and len([char for char in pwd if char not in '$#@1234567890' and char.isupper()==False and char.islower()==False])==0:
        return True
    else:
        return False
...
def remove_avg(a: list) -> list:
    return [a[i] for i in range(1, len(a)) if a[i]>2*sum(a[:i])/len(a[:i])]
...
def frequency(a: list) -> list:
    ...
    no_dupl = set(a)
    for n in no_dupl:
        output.append(tuple([n, a.count(n)]))
...
def is_sorted(a: list) -> bool:
    isCres = len(list(filter(lambda n: n==1,[a[i]>=a[i-1] for i in range(1, len(a))]))) == len(a)-1
    isDecr = len(list(filter(lambda n: n==1,[a[i]<=a[i-1] for i in range(1, len(a))]))) == len(a)-1
    if len(a)>1:
        return isCres or isDecr
    else:
        return True
